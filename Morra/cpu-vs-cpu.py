...
while player_one.count("V") < 3 and player_two.count("V") < 3:
    ...
    recap.append({
        "Partita":"%s" % str(games),
        "Scelta Giocatore 1": "%s" % player_one_sceglie,
        "Scelta Giocatore 2": "%s" % player_two_sceglie,
        "Giocata Giocatore 1": "%s" % player_one_gioca,
        "Giocata Giocatore 2": "%s" % player_two_gioca,
        "Risultato": "%s" % str(result) 
    })
